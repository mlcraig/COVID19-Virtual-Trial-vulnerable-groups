function [time,sol,solstruc] = COVID_IMMUNE_MODELINSILICO(p,tspan)
...
dR = p.beta*S*V/(1+p.eps_F_I/F_B)-p.rho*p.del_N/(1+(p.IC_50_N*2/N)^p.h_N)*R+p.lam_S*(1-(S+I+D+R)/p.Smax)*R;
...
if t>150
[D dD p.del_N*p.rho*S/(1+(p.IC_50_N*2/N)^p.h_N) p.del_N*p.rho/(1+(p.IC_50_N/N)^p.h_N)*R -p.d_D*D]%+p.rho*p.del_N/(1+(p.IC_50_N/M)^p.h_N)*S+p.rho*p.del_N/(1+(0.009/MPhi_I)^p.h_N)*S;%close+p.d_M*M+p.d_N*N+p.d_T*T;
end
